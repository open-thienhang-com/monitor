...
func (ctx *Context) BindJSON(data interface{}) error {
	if ctx.Request.Body != nil {
		b, err := ioutil.ReadAll(ctx.Request.Body) // ioutil change to io
		if err == nil {
...
func (ctx *Context) PjaxUrl(url string) {
	ctx.Response.Header.Add(utils.PjaxUrlHeader, url)
}

// IsPjax check request is pjax or not.
func (ctx *Context) IsPjax() bool {
	return ctx.Headers(utils.PjaxHeader) == "true"
}
...
func (ctx *Context) ServeContent(content io.ReadSeeker, filename string, modtime time.Time, gzipCompression bool) error {
	...
	if ctx.GetContentType() == "" {
		if strings.HasSuffix(filename, ".js") {
			ctx.SetContentType("text/javascript") //TODO:
		} else if strings.HasSuffix(filename, ".css") {
			ctx.SetContentType("text/css") //TODO:
		} else {
			ctx.SetContentType("N?A") //TODO:
		}
...
// PATCH is a shortcut for app.AppendReqAndResp(url, "put", handler).
func (app *App) PATCH(url string, handler ...Handler) *App {
	app.routeANY = false
	app.AppendReqAndResp(url, "patch", handler)
	return app
}
...
func (app *App) Name(name string) {
	if app.routeANY {
		app.Routers[name] = Router{
			Methods: []string{"POST", "GET", "DELETE", "PUT", "PATCH", "OPTIONS", "HEAD"},
...
// PATCH is a shortcut for app.AppendReqAndResp(url, "patch", handler).
func (g *RouterGroup) PATCH(url string, handler ...Handler) *RouterGroup {
	g.app.routeANY = false
	g.AppendReqAndResp(url, "pathc", handler)
	return g
}
...
func (g *RouterGroup) ANY(url string, handler ...Handler) *RouterGroup {
	...
	g.AppendReqAndResp(url, "delete", handler)
	g.AppendReqAndResp(url, "put", handler)
	g.AppendReqAndResp(url, "patch", handler)
	g.AppendReqAndResp(url, "options", handler)
	g.AppendReqAndResp(url, "head", handler)
	return g
}
