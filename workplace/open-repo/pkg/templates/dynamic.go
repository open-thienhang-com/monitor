...
// DynamicRender type
type DynamicRender map[string]*builder
...
// NewRenderer allows create an agnostic theme renderer
...
func (tb builder) buildTemplate() *template.Template {
	switch tb.buildType {
	case templateType:
		return tb.tmpl
...
func (r DynamicRender) Add(name string, tmpl *template.Template) {
	...
	builder := &builder{templateName: name, tmpl: tmpl}
...
func (r DynamicRender) AddFromFiles(name string, files ...string) *template.Template {
	builder := &builder{templateName: name, files: files}
...
func (r DynamicRender) AddFromGlob(name, glob string) *template.Template {
	builder := &builder{templateName: name, glob: glob}
...
func (r DynamicRender) AddFromString(name, templateString string) *template.Template {
	builder := &builder{templateName: name, templateString: templateString}
...
func (r DynamicRender) AddFromFilesFuncs(name string, funcMap template.FuncMap, files ...string) *template.Template {
	...
	builder := &builder{templateName: tname, funcMap: funcMap, files: files}
